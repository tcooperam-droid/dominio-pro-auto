...
export default function FuncionariosPage() {
  ...
  const handleToggleActive = async (emp: Employee) => {
    ...
    if (!confirm(`${nextActive ? "Reativar" : "Desativar"} "${emp.name}"?`)) return;
    try {
      await employeesStore.update(emp.id, { active: nextActive });
      toast.success(`Funcionário ${nextActive ? "reativado" : "desativado"}`);
      setRefreshKey(k => k + 1);
    } catch { toast.error(`Erro ao ${action} funcionário`); }
  };

  const handleDelete = async (emp: Employee) => {
    if (!confirm(`Excluir permanentemente o cadastro de "${emp.name}"? Essa ação não pode ser desfeita.`)) return;
    try {
      await employeesStore.delete(emp.id);
      toast.success("Funcionário excluído");
      setRefreshKey(k => k + 1);
    } catch {
      toast.error("Não foi possível excluir. Se houver agendamentos ou vínculos históricos, desative o funcionário em vez de excluir.");
    }
  };
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{employeesStore.list(false).filter(e => e.active).length} ativos · {employeesStore.list(false).filter(e => !e.active).length} inativos</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => setShowInactive(value => !value)} className="gap-2 text-xs">
            {showInactive ? <UserX className="w-3.5 h-3.5" /> : <UserCheck className="w-3.5 h-3.5" />}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-wrap justify-end gap-1">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className={`h-7 gap-1 px-2 text-xs ${emp.active ? "text-destructive hover:text-destructive" : "text-emerald-400 hover:text-emerald-400"}`}
                      onClick={() => void handleToggleActive(emp)}
                    >
                      {emp.active ? <UserX className="w-3.5 h-3.5" /> : <RotateCcw className="w-3.5 h-3.5" />}
          ...
          <div className="space-y-4 py-2">
            ...
            {editingId && (
              <div className="rounded-md border border-primary/30 bg-primary/5 p-3">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Desative para retirar da agenda de novos agendamentos. O histórico contábil permanece preservado.</p>
              </div>
            )}
          </div>
          <DialogFooter>
