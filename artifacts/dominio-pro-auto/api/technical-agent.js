...
const CORE_REPOSITORY_FILES = [
  "src/lib/agentV2.ts",
  "src/lib/agentContracts.ts",
  "src/lib/agentSchedule.ts",
  "src/lib/schedulerExecutor.ts",
  "src/lib/store/appointmentHolds.ts",
  "src/features/agente-pessoal/service.ts",
  "src/features/agente-pessoal/prompt.ts",
  "src/features/agente-pessoal/bridge.ts",
  "api/agent.js",
  "api/personal-agent.js",
  "migrations/202609132100_scheduler_executor_rpc.sql",
];
...
async function readRepository(question) {
  ...
  const allPaths = (tree.tree || [])
    ...
    .map((item) => item.path)
    .filter((path) => /^(src|api|public|migrations)\//.test(path));
  const scored = allPaths
    .sort((a, b) => scorePath(b, question) - scorePath(a, question));
  const prioritized = [
    ...CORE_REPOSITORY_FILES.filter((path) => allPaths.includes(path)),
    ...scored,
  ];
  const paths = [...new Set(prioritized)].slice(0, MAX_FILES);
